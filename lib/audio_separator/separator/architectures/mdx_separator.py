...
class MDXSeparator(CommonSeparator):
    ...
    def separate(self, audio_data):
        ...
        """
        self.audio_file_path = audio_data.path
        self.audio_file_base = os.path.splitext(os.path.basename(audio_data.path))[0]

        # Prepare the mix for processing
        self.logger.debug(f"Preparing mix for input audio file {self.audio_file_path}...")
        mix = self.prepare_mix(audio_data.data)
        ...
        if not self.output_single_stem or self.output_single_stem.lower() == self.secondary_stem_name.lower():
            ...
            output_sources[self.secondary_stem_name] = self.secondary_source
        ...
        if not self.output_single_stem or self.output_single_stem.lower() == self.primary_stem_name.lower():
            ...
            output_sources[self.primary_stem_name] = self.primary_source
