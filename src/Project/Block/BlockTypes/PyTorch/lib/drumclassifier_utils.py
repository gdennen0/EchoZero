...
class DrumClassifier:
    ...
    def extract_event_data_and_transform(self, event):
        """ extract event data and transform to MEL coefficients"""
        y = event.data.get()
        sr = event.data.get_sr()
        if y is not None:
            return self.transform(y,sr)
        else:
            return None
    ...
    def load_eventData(self, event_data_list):
        dProcessed_spectrograms = {}
        for event_data in event_data_list:
            for item in event_data.items:
                M = self.extract_event_data_and_transform(item)
                if M is not None:
                    dProcessed_spectrograms[item.name] = M
        return dProcessed_spectrograms
