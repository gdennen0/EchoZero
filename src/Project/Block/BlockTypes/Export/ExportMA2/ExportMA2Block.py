...
class ExportMA2Block(Block):
    ...
    def __init__(self, export_dictionary: dict = None):
        ...
        self.command.add("show_filepath", self.show_filepath)
  ...
  # If there is no thumbdrive or default folder, raise an error
    ...
    def set_ma_folder(self, folder=None):
        if folder == None:
            folder = prompt("Enter the MA folder: ")

        self.ma_folder = folder
        Log.info(f"Set the MA export folder to: {folder}")
    
    def set_export_type(self):
        self.export_type = prompt_selection("Enter the export type: ", { "hit" : "hit", "sequence" : "sequence"})

    def change_filename(self, file_name=None):
        if file_name == None:
            file_name = prompt("Enter the song filename prefix: ")
        self.export_preface = file_name
        Log.info(f"Set the MA export prefix to {file_name}")

    def show_filepath(self):
        export_name = f"{self.ma_folder}/{self.export_preface}_timecode_{self.tc_pool}.xml"
        Log.info(f"export file_path: {export_name}")
    ...
    def _save_timecode_xml(self):
        ...
        export_name = f"{self.ma_folder}/{self.export_preface}_timecode_{self.tc_pool}.xml"
        Log.info(f"attempting to export tc xml to {export_name}")
    ...
    def load(self, block_dir):
        ...
        self.set_ma_folder(folder=block_metadata.get("ma_folder"))
