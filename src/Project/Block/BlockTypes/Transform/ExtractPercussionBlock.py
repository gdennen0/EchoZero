...
class ExtractPercussionBlock(Block):
    ...
    def process(self, input_data):
        processed_data = []
        Log.info(f"Extract Percussive Start Sequence Executed")
        ...
        for audio_object in input_data:
            if isinstance(audio_object, AudioData):
                ...
                # Create a new AudioData object for percussive data
                percussive_audio = AudioData()
                percussive_audio.set_name(f"{audio_object.name}_percussive")
                percussive_audio.set_sample_rate(sr)
                percussive_audio.set_data(percussive)

                processed_data.append(percussive_audio)
                Log.info(f"Percussive component extracted for: {audio_object.name}")
