...
class Config:
    ...
    model_name: str = "EZ_model_MFCC"

class MFCCTrain(Block):
    """
    ...
    def process(self, event_data_list):
        ...
        for event_data in event_data_list:
            for event_item in event_data.items:
                ...
                Log.info(f'LENGTH: {length}, SHAPE: {event_item.data.get().shape}')
    ...
    def preprocess_data(self, waveform):
        """
        Preprocess data to be used for training or prediction.
        Generates an STFT spectrogram from the input waveform.
        """
        # Generate STFT
        stft = librosa.stft(waveform, n_fft=self.n_fft, hop_length=self.hop_length)
        # Convert to magnitude spectrogram
        spectrogram = np.abs(stft)
        Log.info(f'SHAPE: {spectrogram.shape}')
