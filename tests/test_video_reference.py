...
def test_project_storage_persists_video_trim_duration_and_loop(tmp_path, monkeypatch) -> None:
    def fake_import_video(_source_path: Path, _working_dir: Path) -> ImportedVideo:
        fake = _FakeImportedVideo()
        return ImportedVideo(
            video_file=fake.video_file,
    ...
    try:
        ...
        storage.import_or_replace_song_video(song.id, tmp_path / "a.mov")

        storage.set_song_video_placement(
            version.id,
            video_start_seconds=3.0,
            video_trim_start_seconds=1.25,
            video_visible_duration_seconds=9.5,
            video_loop_enabled=True,
        )
        placement = storage.song_video_placements.get(version.id)

        assert placement is not None
        assert placement.video_start_seconds == 3.0
        assert placement.video_trim_start_seconds == 1.25
        assert placement.video_visible_duration_seconds == 9.5
        assert placement.video_loop_enabled is True
    finally:
        storage.close()


def test_project_storage_preserves_oversized_loop_duration(tmp_path, monkeypatch) -> None:
    def fake_import_video(_source_path: Path, _working_dir: Path) -> ImportedVideo:
        fake = _FakeImportedVideo()
        return ImportedVideo(
            video_file=fake.video_file,
    ...
    try:
        ...
        storage.set_song_video_placement(
            ...
            video_start_seconds=3.0,
            video_trim_start_seconds=1.25,
            video_visible_duration_seconds=99.0,
            video_loop_enabled=True,
        )
        placement = storage.song_video_placements.get(version.id)

        assert placement is not None
        assert placement.video_visible_duration_seconds == 99.0
        assert placement.video_loop_enabled is True
    finally:
        storage.close()
...
def test_video_mapping_preserves_missing_video_as_offline_reference(tmp_path: Path) -> None:
    missing_path = tmp_path / "missing.mov"

    mapping = video_mapping_from_presentation(_video_presentation(missing_path))

    assert mapping is not None
    assert mapping.media_available is False
    assert "Missing video file" in mapping.unavailable_reason


def test_video_mapping_uses_trimmed_media_start_and_visible_duration(tmp_path: Path) -> None:
    video_path = tmp_path / "ref.mov"
    video_path.write_bytes(b"video")

    presentation = _video_presentation(
        video_path,
    ...
    assert mapping is not None
    assert mapping.media_seconds_for_song_time(1.0) == 2.0
    assert mapping.media_seconds_for_song_time(3.0) == pytest.approx(3.5)
    assert mapping.contains_song_time(5.5)
    assert not mapping.contains_song_time(5.6)
...
def test_video_window_clears_stale_frame_on_status_changes() -> None:
    source = (_REPO_ROOT / "echozero/ui/qt/video_window.py").read_text()

    assert "self._image = QImage()\n        self.update()" in source
    assert "self._player.setSource(QUrl())" in source
    assert 'self._video_widget.set_status("No video reference")' in source
    assert 'self._video_widget.set_status("Loading video reference")' in source
    assert "Missing video file" in source


def test_song_switch_lifecycle_syncs_open_video_preview() -> None:
    source = (_REPO_ROOT / "echozero/ui/qt/app_shell_project_lifecycle.py").read_text()

    assert source.count("shell._sync_video_playback_from_presentation()") >= 2
    assert "shell._app.runtime_video = video_controller" in source


def test_video_actions_route_to_layer_scoped_runtime_methods() -> None:
    action_source = (
        _REPO_ROOT / "echozero/ui/qt/timeline/widget_action_contract_mixin.py"
    ).read_text()
    widget_source = (_REPO_ROOT / "echozero/ui/qt/timeline/widget.py").read_text()

    assert 'getattr(runtime, "import_or_replace_video_layer", None)' in action_source
    assert 'getattr(runtime, "remove_video_layer", None)' in action_source
    assert 'getattr(runtime, "set_video_layer_placement", None)' in action_source
    assert 'getattr(runtime, "set_video_layer_placement", None)' in widget_source
    assert "set_active_song_video_start_seconds" not in widget_source
...
def _video_presentation(
    ...
    *,
    ...
    visible_duration_seconds: float = 8.0,
    ...
    return TimelinePresentation(
        ...
        layers=[
            LayerPresentation(
                ...
                video_trim_start_seconds=trim_start_seconds,
                video_duration_seconds=8.0,
                video_visible_duration_seconds=visible_duration_seconds,
