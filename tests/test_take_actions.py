...
def _take(take_id: str, *, run_id: str, is_main: bool) -> Take:
    return Take(
        ...
        origin="pipeline",
        source=TakeSource(
            block_id="b1", block_type="SeparateAudio", settings_snapshot={}, run_id=run_id
        ),
...
def test_promote_take_to_main_marks_dependents_stale(tmp_path):
    ...
    sync_layer_object_content(
        ...
        takes=session.takes.list_by_layer(parent.id),
    )
    session.commit()

    updated_parent, updated_layers = promote_take_to_main(
        session, layer_id="drums", take_id="take2"
    )

    assert updated_parent.provenance["current_main_take_id"] == "take2"
    drums_object = session.timeline_objects.get(object_id_for_layer("drums"))
    assert drums_object is not None
    assert drums_object.main_content_id == content_id_for_take("take2")
    assert [
        candidate.content_id
        for candidate in session.object_candidates.list_by_object(drums_object.id)
    ] == [content_id_for_take("take1")]
