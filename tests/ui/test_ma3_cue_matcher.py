...
def test_ma3_cue_matcher_creates_missing_cues_from_empty_rows() -> None:
    ...
    try:
        assert [mapping.cue_number for mapping in dialog.selected_mappings()] == [1]

        dialog._create_missing_cues_from_ez_rows()

        mappings = dialog.selected_mappings()
        assert [mapping.cue_number for mapping in mappings] == [1, 2, 5]
...
def test_ma3_cue_matcher_parses_typed_section_cue_refs() -> None:
    ...
    assert MA3CueMatcherDialog._parse_combo_cue_text("1000 - Intro") == 1
    assert MA3CueMatcherDialog._parse_combo_cue_text("Cue 1000") == 1
