...
def test_app_shell_runtime_new_song_after_reopen_uses_saved_pipeline_defaults():
    temp_root = _repo_local_temp_root()
    save_path = temp_root / "session-save-defaults-new-song.ez"
    runtime = build_app_shell(
        working_dir_root=temp_root / "working",
        analysis_service=build_mock_analysis_service(),
    )
    ...
    try:
        first_audio_path = write_test_wav(temp_root / "fixtures" / "saved-defaults-source.wav")
        second_audio_path = write_test_wav(temp_root / "fixtures" / "saved-defaults-target.wav")
        runtime.add_song_from_path("Defaults Source", first_audio_path)
        runtime.save_project_as(save_path)
        ...
        runtime.dispatch_object_action_command(session.session_id, SaveSessionToDefaults())

        runtime.save_project()
        runtime.open_project(save_path)
        runtime.add_song_from_path("Defaults Target", second_audio_path)
