...
def test_contract_set_first_beat_here_calls_runtime():
    app = QApplication.instance() or QApplication([])
    ...
    class _Runtime:
        ...
        def set_song_version_beat_anchor_seconds(
            ...
            self.calls.append((song_version_id, beat_anchor_seconds))
            self._presentation = replace(
                self._presentation,
                active_song_version_id=song_version_id,
    ...
    try:
        contract = build_timeline_inspector_contract(
            widget.presentation,
            hit_target=TimelineInspectorHitTarget(kind="timeline", time_seconds=2.75),
        )
        action = next(
            action
            for section in contract.context_sections
            if section.section_id == "transport"
            for action in section.actions
            if action.action_id == "song.version.set_first_beat_here"
        )

        widget._trigger_contract_action(action)

        assert runtime.calls == [("song_version_festival", 2.75)]
        assert widget.presentation.beat_anchor_seconds == 2.75
    finally:
        widget.close()
        app.processEvents()


def test_contract_set_first_beat_to_playhead_calls_runtime():
    app = QApplication.instance() or QApplication([])
    ...
    try:
        action = next(
            ...
            for section in build_timeline_inspector_contract(widget.presentation).context_sections
            for action in section.actions
            if action.action_id == "song.version.set_first_beat_to_playhead"
        )

        widget._trigger_contract_action(action)

        assert runtime.calls == [("song_version_festival", 3.5)]
        assert widget.presentation.beat_anchor_seconds == 3.5
    finally:
        widget.close()
        app.processEvents()
