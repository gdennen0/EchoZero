...
def _build_dense_timeline(*, layer_count: int = 10, events_per_main: int = 1200) -> Timeline:
    ...
    for layer_idx in range(layer_count):
        ...
        main_events = [
            _event(f"m_{layer_idx}_{i}", main_take_id, i * spacing) for i in range(events_per_main)
        ]
        alt_events = [
            _event(f"a_{layer_idx}_{i}", alt_take_id, (i * spacing) + 0.02)
            for i in range(max(100, events_per_main // 4))
        ]
        ...
        layer = Layer(
            ...
            takes=[
                Take(
                    id=TakeId(main_take_id),
                    layer_id=LayerId(f"layer_{layer_idx}"),
                    name="Main",
                    events=main_events,
                ),
                Take(
                    id=TakeId(alt_take_id),
                    layer_id=LayerId(f"layer_{layer_idx}"),
                    name="Take 2",
                    events=alt_events,
                ),
...
def benchmark_assemble_cached(*, iterations: int = 180) -> dict[str, float]:
    ...
    try:
        for i in range(20):
            session.transport_state.playhead = float(i) * 0.05
            assembler.assemble(timeline, session)
...
def main() -> None:
    ...
    parser.add_argument(
        "--strict", action="store_true", help="Exit non-zero when thresholds are exceeded"
    )
    parser.add_argument(
        "--json-out", type=str, default="", help="Optional path to write benchmark JSON"
    )
    ...
    payload = run(
        strict=args.strict, thresholds_path=Path(args.thresholds) if args.thresholds else None
    )
