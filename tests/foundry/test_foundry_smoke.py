...
def test_artifact_compatibility_rejects_class_order_mismatch(tmp_path: Path):
    ...
    run_spec = {
        ...
        "training": {"epochs": 1, "batchSize": 2, "learningRate": 0.01, "seed": 37},
    }

    run = app.create_run(version.id, run_spec)
    run = app.start_run(run.id)
    bad_classes = list(reversed(version.class_map))
    artifact = app.finalize_artifact(
        run.id,
        {
            "weightsPath": "model.pth",
    ...
    report = app.validate_artifact(artifact.id)
    assert report.ok is False
    assert "manifest.classes must match dataset version class_map order" in report.errors


def test_artifact_compatibility_rejects_preprocessing_mismatch(tmp_path: Path):
    samples = tmp_path / "samples"
    write_percussion_dataset(samples)
    ...
    run = app.create_run(version.id, run_spec)
    run = app.start_run(run.id)
    bad_pre = dict(run_spec["data"])
    bad_pre["hopLength"] = 256
    artifact = app.finalize_artifact(
        run.id,
        {
            "weightsPath": "model.pth",
            ...
            "classificationMode": "multiclass",
            "runtime": {"consumer": "PyTorchAudioClassify", "backend": "pytorch", "device": "cpu"},
            "inferencePreprocessing": bad_pre,
        },
    )

    report = app.validate_artifact(artifact.id)
    assert report.ok is False
    assert "manifest.inferencePreprocessing.hopLength must match run spec" in report.errors
