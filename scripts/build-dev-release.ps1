...
$buildRoot = Join-Path $repoRoot "build/dev-release"
$distRoot = Join-Path $repoRoot "dist/dev-release"
$preferredArtifactsRoot = Join-Path $repoRoot "artifacts/releases/dev"
$fallbackArtifactsRoot = Join-Path $repoRoot "build/dev-release/releases/dev"
$artifactsRoot = $preferredArtifactsRoot
$releaseRoot = Join-Path $artifactsRoot $timestamp
$releaseAppDir = Join-Path $releaseRoot "EchoZeroDev"
...
if ($Clean) {
    $pyInstallerArgs = @("-m", "PyInstaller", "--noconfirm", "--clean", "--windowed", "--name", "EchoZeroDev", "--workpath", $buildRoot, "--specpath", $buildRoot, "--distpath", $distRoot, "--add-data", "${timelineFixtureDir};echozero/ui/qt/timeline/fixtures", "run_echozero.py")
}

Write-Host "Building EchoZero development release..."
...
$builtAppDir = Join-Path $distRoot "EchoZeroDev"
...
Write-Host "BUILD DEVELOPMENT RELEASE COMPLETE"
