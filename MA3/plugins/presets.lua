...
local function previewStoreCueOnlyPayload(sequenceNo, targetCueNo, requestId)
    ...
    local warnings = {}
    local unsupportedReasons = {
        "Recipe Store Cue Only is analysis-only until recipe-line authoring/restoration is implemented; do not call native MA Store/CueOnly.",
    }
    ...
    if actualTargetCueNumber then
        ...
        targetLocalLines = {}
        table.insert(warnings, "Target cue does not exist yet; recipe-only store will need recipe-line creation logic.")
    end
    ...
    if nextCueValue then
        ...
        table.insert(warnings, "Target cue has no following cue; recipe-only restore has no next cue target.")
    end
    ...
    local payload = {
        ...
        supported = false,
        status = "recipe_only_planned",
        operation = "recipe_store_cue_only",
        command = nil,
        recipe_only = true,
        warnings = dedupeTextList(warnings),
        unsupported_reasons = dedupeTextList(unsupportedReasons),
        ...
        note = "Recipe Store Cue Only must be implemented by writing recipe lines from programmer-derived recipe data and restoring recipe tracking in the following cue. Native Store/CueOnly is intentionally not used.",
    }
    if requestId ~= nil then
        payload.request_id = requestId
    end
...
function EZ.PreviewRecipeCueOnly(sequenceNo, sourceCueNo, targetCueNo, requestId)
    ...
    if payload then
        ...
        payload.note = "PreviewRecipeCueOnly is a deprecated compatibility preview for recipe Store Cue Only. Source cue is ignored; no native MA Store/CueOnly command is used."
...
function EZ.ApplyRecipeCueOnly(sequenceNo, sourceCueNo, targetCueNo, requestId)
    ...
    if payload then
        ...
        payload.source_cue_no = cueNumberValue(normalizeCueNumberText(sourceCueNo))
        payload.note = "ApplyRecipeCueOnly is blocked. Recipe Store Cue Only needs recipe-line authoring/restoration, not native MA Store/CueOnly."
...
function EZ.CopyCueWithStatus(sequenceNo, sourceCueNo, destCueNo, requestId)
    ...
    payload.supported = false
    payload.status = "unsupported"
    payload.recipe_only = true
    payload.unsupported_reasons = dedupeTextList({
        "Recipe Copy Cue Status is analysis-only until recipe-line authoring/restoration is implemented; do not call native MA Copy/CueOnly/CopyCueSource Status.",
    })
    payload.note = "Recipe Copy Cue Status must be implemented by copying resolved recipe contributors into destination recipe lines and restoring recipe tracking as needed. Native MA Copy/CueOnly is intentionally not used."
