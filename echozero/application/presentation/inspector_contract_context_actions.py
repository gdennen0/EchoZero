    ...
    *,
    ...
    if hit_target is not None and hit_target.time_seconds is not None:
        ...
        if (
            ...
            and presentation.bpm is not None
            and float(presentation.bpm) > 0.0
        ):
            transport_actions.append(
                InspectorAction(
                    action_id="song.version.set_first_beat_here",
                    label="Set First Beat Here",
                    group="transport",
                    params={
                        "song_version_id": presentation.active_song_version_id,
                        "beat_anchor_seconds": hit_target.time_seconds,
                    },
                )
            )
        sections.append(
            InspectorContextSection(
                ...
                label="Transport",
                actions=tuple(transport_actions),
            )
        )
    ...
    if layer is not None:
        ...
        if layer.kind not in {LayerKind.EVENT, LayerKind.REFERENCE}:
            layer_actions = [
                InspectorAction(
...
def song_context_actions(
    ...
    if presentation.active_song_version_id:
        ...
        if presentation.bpm is not None and float(presentation.bpm) > 0.0:
            actions.append(
                InspectorAction(
                    action_id="song.version.set_first_beat_to_playhead",
                    label="Set First Beat To Playhead",
                    group="song",
                    params={"song_version_id": presentation.active_song_version_id},
                )
            )
