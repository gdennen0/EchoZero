...
from echozero.ui.qt.timeline.demo_app import DemoTimelineApp, build_demo_app, build_real_data_demo_app
...
DEFAULT_REAL_APP_REFERENCE_ROOT = DEFAULT_REFERENCE_ROOT / "real-app-path"
...
def _resize_widget_for_presentation(widget: TimelineWidget, presentation: TimelinePresentation) -> None:
    widget.resize(1440, estimate_full_window_height(presentation))
    widget.set_presentation(presentation)
    _app().processEvents()


def _set_runtime_presentation(widget: TimelineWidget, runtime: AppShellRuntime | DemoTimelineApp) -> TimelinePresentation:
    presentation = runtime.presentation()
    _resize_widget_for_presentation(widget, presentation)
    return presentation
...
def collect_reference_artifacts(*, run_folder: Path, reference_root: Path = DEFAULT_REFERENCE_ROOT) -> list[ScenarioResult]:
    specs = [
        (
            "reference_inclusion",
...
def include_packaged_app_evidence(*, run_folder: Path, releases_root: Path = DEFAULT_RELEASES_ROOT) -> ScenarioResult:
...
def run_canonical_app_lifecycle(*, run_folder: Path, record: bool, fps: int) -> list[ScenarioResult]:
    app = _app()
...
def run_fixture_rich_editing_flow(*, run_folder: Path, record: bool, fps: int) -> list[ScenarioResult]:
    app = _app()
    demo = build_demo_app()
...
def run_real_data_scenario(*, run_folder: Path, audio_path: Path | None, record: bool, fps: int) -> list[ScenarioResult]:
...
def run_demo_suite(
    *,
    ...
    scenario_results.extend(run_canonical_app_lifecycle(run_folder=run_folder, record=record, fps=fps))
    scenario_results.extend(run_fixture_rich_editing_flow(run_folder=run_folder, record=record, fps=fps))
    scenario_results.extend(collect_reference_artifacts(run_folder=run_folder))
    scenario_results.append(include_packaged_app_evidence(run_folder=run_folder))
    scenario_results.extend(run_real_data_scenario(run_folder=run_folder, audio_path=audio_path, record=record, fps=fps))
