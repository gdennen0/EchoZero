...
class TrainerBackendFactory:
    ...
    def resolve(self, run_spec: dict, *, baseline_backend: TrainerBackend) -> TrainerBackend:
        ...
        return builder(run_spec, baseline_backend)
    ...
    @staticmethod
    def _build_baseline(run_spec: dict, baseline_backend: TrainerBackend) -> TrainerBackend:
        del run_spec
        return baseline_backend

    @staticmethod
    def _build_cnn(run_spec: dict, baseline_backend: TrainerBackend) -> TrainerBackend:
        del run_spec
        root = getattr(baseline_backend, "_root", None)
        if root is None:
            raise ValueError("baseline backend must expose a root path for cnn backend resolution")
        try:
    ...
    @staticmethod
    def _build_crnn(run_spec: dict, baseline_backend: TrainerBackend) -> TrainerBackend:
        del run_spec
        root = getattr(baseline_backend, "_root", None)
        if root is None:
            raise ValueError(
                "baseline backend must expose a root path for crnn backend resolution"
            )
        try:
