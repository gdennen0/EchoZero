...
class SplitBalanceService:
    def plan_splits(
        ...
        *,
        ...
        groups = [
            (group_key, sorted(samples, key=lambda sample: sample.sample_id))
            for group_key, samples in grouped_samples.items()
        ]
        rng.shuffle(groups)
        target_test = {
            label: int(round(count * test_split)) for label, count in class_counts.items()
        }
        target_val = {
            label: int(round(count * validation_split)) for label, count in class_counts.items()
        }
        ...
        for index, (_, samples) in enumerate(groups):
            ...
            split_name = "train"
            if remaining_groups > 1 and self._should_assign(
                label_counts, assigned_test, target_test
            ):
                split_name = "test"
                for label, count in label_counts.items():
                    assigned_test[label] += count
            elif remaining_groups > 1 and self._should_assign(
                label_counts, assigned_val, target_val
            ):
                split_name = "val"
                for label, count in label_counts.items():
            ...
            self._assign_samples(
                sample_ids, split_name, train=train, val=val, test=test, assignments=assignments
            )
    ...
    def validate_split_plan(cls, version: DatasetVersion, split_plan: dict) -> dict:
        ...
        if split_plan.get("dataset_manifest_hash") not in {None, version.manifest_hash}:
            errors.append(
                "split plan dataset_manifest_hash does not match dataset version manifest_hash"
            )
        if split_plan.get("dataset_sample_count") not in {None, len(version.samples)}:
            errors.append(
                "split plan dataset_sample_count does not match dataset version sample count"
            )
        ...
        for split_name in ("train", "val", "test"):
            ...
            invalid = [
                sample_id for sample_id in split_ids if assignments.get(sample_id) != split_name
            ]
            if invalid:
        ...
        recomputed = cls._build_reproducibility_report(
            version, assignments, seed=int(split_plan.get("seed", 42))
        )
        for key in ("group_fingerprint", "assignment_fingerprint"):
            ...
            if expected is not None and expected != recomputed[key]:
                errors.append(
                    f"split plan reproducibility.{key} does not match current dataset state"
                )

        if (
            split_plan.get("planner") == "content_hash_grouped_v1"
            and split_plan.get("policy") is None
        ):
    ...
    def _build_label_distribution(version: DatasetVersion, assignments: dict[str, str]) -> dict:
        ...
        for sample in version.samples:
            ...
            distribution[split_name][sample.label] = (
                distribution[split_name].get(sample.label, 0) + 1
            )
        return distribution

    @classmethod
    def _build_group_distribution(
        cls, version: DatasetVersion, assignments: dict[str, str]
    ) -> dict[str, int]:
    ...
    def _build_leakage_report(cls, version: DatasetVersion, assignments: dict[str, str]) -> dict:
        ...
        duplicate_hashes = sorted(
            content_hash for content_hash, splits in hash_splits.items() if len(splits) > 1
        )
    ...
    def _build_reproducibility_report(
        cls, version: DatasetVersion, assignments: dict[str, str], *, seed: int
    ) -> dict:
        ...
        group_rows = sorted(
            (group_id, sorted(sample_ids)) for group_id, sample_ids in grouped_sample_ids.items()
        )
        group_fingerprint = hashlib.sha256(
            json.dumps(group_rows, sort_keys=True).encode("utf-8")
        ).hexdigest()
        assignment_rows = sorted(assignments.items())
        assignment_fingerprint = hashlib.sha256(
            json.dumps(assignment_rows, sort_keys=True).encode("utf-8")
        ).hexdigest()
        return {
            "seed": seed,
    ...
    @staticmethod
    def _should_assign(
        label_counts: dict[str, int], assigned: dict[str, int], targets: dict[str, int]
    ) -> bool:
