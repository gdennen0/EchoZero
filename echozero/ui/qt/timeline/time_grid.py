...
_GRID_MODE_LABELS = {
    TimelineGridMode.OFF: "Off",
    TimelineGridMode.AUTO: "Time",
    TimelineGridMode.BEAT: "Beats",
}
...
def grid_mode_label(mode: TimelineGridMode | str) -> str:
    """Return the operator-facing label for one grid mode."""

    normalized_mode = _normalize_grid_mode(mode)
    return _GRID_MODE_LABELS.get(normalized_mode, normalized_mode.value.title())
...
def visible_grid_lines(
    *,
    ...
    if normalized_mode is TimelineGridMode.BEAT and bpm and bpm > 0:
        start_index = int(floor((start_second - beat_anchor) / step)) - 1
    ...
    for index in range(start_index, max(start_index, end_index) + 1):
        ...
        if normalized_mode is TimelineGridMode.BEAT and bpm and bpm > 0:
            beat_seconds = 60.0 / float(bpm)
            relative_time = time_seconds - beat_anchor
